(function(A,p){function u(){x+=1;return"i"+x}function w(a,b){var d,g,h,f;d=b.split(".");f=a;g=0;for(h=d.length;g<h;g+=1){f=f[d[g]];if(f===p||f===null)return p}return f}function y(a,b,d){b=b.split(".");var g=b.length-1,h=a;for(a=0;a<g;a+=1){h=h[b[a]];if(h===p)return}h[b[g]]=d}function z(a,b){var d,g=a.length,h,f;for(d=0;d<g;d+=1)if(typeof a[d].length==="number"){h=0;for(f=a[d].length;h<f;h+=1)b(a[d][h])}else b(a[d])}function v(a,b){var d;a=a||"";for(d in b)if(b.hasOwnProperty(d))a=a.replace(RegExp("{"+
d+"}","g"),b[d]);return a}var k={version:"0.0.1"},x=0,B=/^\s+/,C=/\s+$/,s={notUniqueReceiverId:"engine.addReceiver: recevier with given ID has been already added.",receiverIdMustBeString:"engine.addReceiver: id must be string",noReceiveMessageMethod:'engine.addReceiver: receiver should have method "receiveMessage" or be a function',noReceiverId:"engine.addRule: rule must have receiverId property, type string",receiverNotFound:"engine.sendMessage: receiver not found.",elementWithoutBinding:"element.notifyValueChange: can't send notification if binding property not defined.",

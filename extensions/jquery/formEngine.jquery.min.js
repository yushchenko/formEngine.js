...
b.engine.addRules(b.provider.getRules());b.engine.addTriggers(b.provider.getTriggers());b.view.initialize();b.model.set(a);return b};fe.jquery.element=function(d){var a=fe.element(d),b,h;a.editorId=a.id;a.containerId="c_"+a.id;a.initialize=function(){a.doInitialize&&a.doInitialize();a.eachChild(function(f){f.initialize()})};a.getMarkup=function(){var f=a.template,e="";a.eachChild(function(j){e+=j.getMarkup()});if(typeof f==="function")return f({editorId:a.editorId,containerId:a.containerId,content:e,
properties:a.properties});return e};a.getEditor=function(){if(b===l)b=$("#"+a.editorId);return b};a.getContainer=function(){if(h===l)h=$("#"+a.containerId);return h};return a};fe.jquery.elements.view=function(d){var a=fe.jquery.element(d);a.template=k('<div id="<%=containerId%>"><%=content%></div>');a.doInitialize=function(){var b=a.properties.viewContainerId;if(typeof b!=="string")throw Error(q.undefinedViewContainerId);document.getElementById(b).innerHTML=a.getMarkup()};return a};fe.jquery.elements.textBox=
function(d){var a=fe.jquery.element(d);a.template=k('<div id="<%=containerId%>"><label for="<%=editorId%>" class="fe-element-label"><%=properties.label%></label><input type="text" id="<%=editorId%>"></input></div>');a.initialize=function(){a.getEditor().change(function(){a.notifyValueChange(a.getEditor().val())})};a.setValue=function(b){a.getEditor().val(b)};a.setHidden=function(b){a.getContainer().toggleClass("fe-hidden",b)};return a};fe.jquery.elements.comboBox=function(d){function a(c){function g(n){var m=
